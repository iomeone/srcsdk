...
Vector VecCheckToss( CBaseEntity *pEntity, Vector vecSpot1, Vector vecSpot2, float flHeightMaxRatio, float flGravityAdj, bool bRandomize, Vector *vecMins = NULL, Vector *vecMaxs = NULL );
...
CBaseEntity *CBaseCombatCharacter::Weapon_FindUsable( const Vector &range )
{
	...
	for ( int i = 0; i < listCount; i++ )
	{
		...
		if ( pBestWeapon )
		{
			...
			{
#if 1
				float flRatio = MIN( (2.5f / (pWeapon->GetWeight() - (GetActiveWeapon() ? GetActiveWeapon()->GetWeight() : 0))), 1.0f );
				if (flRatio < 0)
					flRatio *= -1; flRatio += 1.0f;
				fCurDist *= flRatio;
#else
				fCurDist *= MIN( (2.5f / pWeapon->GetWeight()), 1.0f );
#endif
			}
