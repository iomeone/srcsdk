...
//-----------------------------------------------------------------------------
class CVoiceStatusHelper : public IVoiceStatusHelper
{
public:
...
static CVoiceStatusHelper g_VoiceStatusHelper;
...
CBaseClientDLL gClientDLL;
IBaseClientDLL *clientdll = &gClientDLL;

EXPOSE_SINGLE_INTERFACE_GLOBALVAR( CBaseClientDLL, IBaseClientDLL, CLIENT_DLL_INTERFACE_VERSION, gClientDLL );
...
void PrecacheMaterial( const char *pMaterialName )
{
	gClientDLL.PrecacheMaterial( pMaterialName );
}
...
//-----------------------------------------------------------------------------
void CBaseClientDLL::ResetStringTablePointers()
{
...
void OnMaterialStringTableChanged( void *object, INetworkStringTable *stringTable, int stringNumber, const char *newString, void const *newData )
{
	...
	gClientDLL.PrecacheMaterial( newString );
...
//-----------------------------------------------------------------------------
void CBaseClientDLL::InstallStringTableCallback( const char *tableName )
{
...
// Material precache
//-----------------------------------------------------------------------------
void CBaseClientDLL::PrecacheMaterial( const char *pMaterialName )
{
	...
	{
		Msg("\n ##### CBaseClientDLL::PrecacheMaterial could not find material %s (%s)", pMaterialName, pTempBuf );
	}
}

void CBaseClientDLL::UncacheAllMaterials( )
{
...
//-----------------------------------------------------------------------------
bool CBaseClientDLL::DispatchUserMessage( int msg_type, bf_read &msg_data )
{
...
void CBaseClientDLL::FrameStageNotify( ClientFrameStage_t curStage )
{
	...
	case FRAME_RENDER_START:
		{
			VPROF( "CBaseClientDLL::FrameStageNotify FRAME_RENDER_START" );
	...
	case FRAME_RENDER_END:
		{
			VPROF( "CBaseClientDLL::FrameStageNotify FRAME_RENDER_END" );
			OnRenderEnd();
	...
	case FRAME_NET_UPDATE_START:
		{
			VPROF( "CBaseClientDLL::FrameStageNotify FRAME_NET_UPDATE_START" );
	...
	case FRAME_NET_UPDATE_POSTDATAUPDATE_START:
		{
			VPROF( "CBaseClientDLL::FrameStageNotify FRAME_NET_UPDATE_POSTDATAUPDATE_START" );
			PREDICTION_STARTTRACKVALUE( "postdataupdate" );
		}
		break;
	case FRAME_NET_UPDATE_POSTDATAUPDATE_END:
		{
			VPROF( "CBaseClientDLL::FrameStageNotify FRAME_NET_UPDATE_POSTDATAUPDATE_END" );
			PREDICTION_ENDTRACKVALUE();
...
void CBaseClientDLL::SaveWriteFields( CSaveRestoreData *pSaveData, const char *pname, void *pBaseData, datamap_t *pMap, typedescription_t *pFields, int fieldCount )
{
	CSave saveHelper( pSaveData );
	saveHelper.WriteFields( pname, pBaseData, pMap, pFields, fieldCount );
}

void CBaseClientDLL::SaveReadFields( CSaveRestoreData *pSaveData, const char *pname, void *pBaseData, datamap_t *pMap, typedescription_t *pFields, int fieldCount )
{
	CRestore restoreHelper( pSaveData );
	restoreHelper.ReadFields( pname, pBaseData, pMap, pFields, fieldCount );
}

void CBaseClientDLL::PreSave( CSaveRestoreData *s )
{
	g_pGameSaveRestoreBlockSet->PreSave( s );
}

void CBaseClientDLL::Save( CSaveRestoreData *s )
{
	CSave saveHelper( s );
	g_pGameSaveRestoreBlockSet->Save( &saveHelper );
}

void CBaseClientDLL::WriteSaveHeaders( CSaveRestoreData *s )
{
	CSave saveHelper( s );
...
void CBaseClientDLL::ReadRestoreHeaders( CSaveRestoreData *s )
{
...
void CBaseClientDLL::Restore( CSaveRestoreData *s, bool b )
{
	CRestore restore(s);
...
// NEW INTERFACES
// save game screenshot writing
void CBaseClientDLL::WriteSaveGameScreenshotOfSize( const char *pFilename, int width, int height, bool bCreatePowerOf2Padded/*=false*/,
...
void CBaseClientDLL::RenderView( const CViewSetup &setup, int nClearFlags, int whatToDraw )
{
	VPROF("RenderView");
...
void CBaseClientDLL::ClientAdjustStartSoundParams( StartSoundParams_t& params )
{
#ifdef TF_CLIENT_DLL
