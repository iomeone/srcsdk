	...
	const char *pLocation )
{
	...
	if ( V_stristr( fullLocationPath, ".vpk" ) == NULL && Q_stricmp( pPathID, "game" ) == 0 )
	{
		...
		if ( bFirstGamePath && *bFirstGamePath )
		{
...
FSReturnCode_t FileSystem_LoadSearchPaths( CFSSearchPathsInit &initInfo )
{
	...
	if ( pszExtraSearchPath )
	{
		...
		{
			char *szSearchPath = vecPaths[ idxExtraPath ];
			Q_StripPrecedingAndTrailingWhitespace( szSearchPath );
			V_FixSlashes( szSearchPath );
			if ( !V_RemoveDotSlashes( szSearchPath ) )
				Error( "Bad -insert_search_path - Can't resolve pathname for '%s'", szSearchPath );
			V_StripTrailingSlash( szSearchPath );
			FileSystem_AddLoadedSearchPath( initInfo, "GAME", NULL, baseDir, szSearchPath );
			FileSystem_AddLoadedSearchPath( initInfo, "MOD", NULL, baseDir, szSearchPath );
		}
	}
	...
	for ( KeyValues *pCur=pSearchPaths->GetFirstValue(); pCur; pCur=pCur->GetNextValue() )
	{
		...
		V_strcpy(szSearchPath, pLocation);

		// Now resolve any ./'s.
		V_FixSlashes( szSearchPath );
		if ( !V_RemoveDotSlashes( szSearchPath ) )
			Error( "FileSystem_AddLoadedSearchPath - Can't resolve pathname for '%s'", szSearchPath );
		V_StripTrailingSlash( szSearchPath );

		if ( Q_stristr( pLocation, GAMEINFOPATH_TOKEN ) == pLocation )
		{
			pLocation += V_strlen( GAMEINFOPATH_TOKEN );
			pszBaseDir = initInfo.m_pDirectoryName;

			FOR_EACH_VEC( vecPathIDs, idxPathID )
			{
				FileSystem_AddLoadedSearchPath( initInfo, vecPathIDs[ idxPathID ], &bFirstGamePath, pszBaseDir, szSearchPath );
			}
		}
		else if ( Q_stristr( pLocation, BASESOURCEPATHS_TOKEN ) == pLocation )
		{
			...
			pLocation += V_strlen( BASESOURCEPATHS_TOKEN );
			...
			FOR_EACH_VEC( vecPathIDs, idxPathID )
			{
				...
				{			
					...
					V_snprintf( ep1EraPath, sizeof( ep1EraPath ), "..%c%s", CORRECT_PATH_SEPARATOR, szSearchPath );
					FileSystem_AddLoadedSearchPath( initInfo, vecPathIDs[ idxPathID ], &bFirstGamePath, pszBaseDir, ep1EraPath );
				}
			}
		}
		...
		V_MakeAbsolutePath( szAbsSearchPath, sizeof( szAbsSearchPath ), szSearchPath, pszBaseDir );
		...
		{
			vecFullLocationPaths.CopyAndAddToTail( szSearchPath );
		}
		else
		{
			...
			if ( pszFoundShortName )
			{
				do 
				{
					...
					if ( pszFoundShortName[0] != '.' && ( initInfo.m_pFileSystem->FindIsDirectory( findHandle ) || V_stristr( pszFoundShortName, ".vpk" ) ) )
					{
						...
						V_ExtractFilePath( szSearchPath, szAbsName, sizeof( szAbsName ) );
							...
							|| !V_stricmp( pszFoundShortName, "models" ) )
						{
							...
							V_ExtractFilePath( szSearchPath, szReadme, sizeof( szReadme ) );
		...
		FOR_EACH_VEC( vecFullLocationPaths, idxLocation )
		{
			FOR_EACH_VEC( vecPathIDs, idxPathID )
			{
				FileSystem_AddLoadedSearchPath( initInfo, vecPathIDs[ idxPathID ], &bFirstGamePath, pszBaseDir, vecFullLocationPaths[ idxLocation ] );
			}
		}
	}
...
FSReturnCode_t FileSystem_GetFileSystemDLLName( char *pFileSystemDLL, int nMaxLen, bool &bSteam )
{
	...
	#endif
	if (
		CommandLine()->FindParm( "-steam" ) || CommandLine()->FindParm( "-steamlocal" ) ||
		#if defined( OSX ) || defined( LINUX )
