...
void CDmeFilmClip::OnConstruction()
{
	...
	m_bIsUsingCachedVersion.Init( this, "useVidFile" );
...
{
	if ( m_hCachedVersion != nullptr )
	{
		g_pVideo->DestroyVideoMaterial( m_hCachedVersion );
		m_hCachedVersion = nullptr;
	}
}
...
{
	BaseClass::Resolve();
	if ( m_VideoFile.IsDirty() )
	{
		m_bReloadCachedVersion = true;
		m_VideoFile.GetAttribute()->RemoveFlag( FATTRIB_DIRTY );
	}
}
...
IVideoMaterial *CDmeFilmClip::GetCachedVideo()
{
	if ( m_bReloadCachedVersion )
	{
		if ( g_pVideo )
		{
			...
			if ( m_VideoFile[0] )
			{
				m_hCachedVersion = g_pVideo->CreateVideoMaterial( m_VideoFile, m_VideoFile, "MOD" );
			}
		}
		m_bReloadCachedVersion = false;
	}
...
void CDmeFilmClip::SetCachedVideo( const char *pVidFile )
{
	m_VideoFile = pVidFile;
	m_bReloadCachedVersion = true;
}
