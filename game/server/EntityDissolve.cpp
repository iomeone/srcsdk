	...
	float flStartTime, int nDissolveType, bool *pRagdollCreated )
{
	...
	{
		if ( pTarget->IsNPC() && pTarget->GetBaseAnimating()->CanBecomeRagdoll() )
		{
			CTakeDamageInfo info;
			CBaseEntity *pRagdoll = CreateServerRagdoll( pTarget->GetBaseAnimating(), 0, info, COLLISION_GROUP_DEBRIS, true );
