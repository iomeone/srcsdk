...
private:
	...
	CUtlMap<Activity,int> m_sequenceCache;	// Cache the first N sequences, since we don't have weights.
...
Activity CCSHostageAnimState::CalcMainActivity()
{
	...
	else
	{
		...
		*/
		...
		Activity idealActivity = ACT_BUSY_QUEUE;
...
CCSPlayerAnimState::CCSPlayerAnimState()
{
	...
	SetDefLessFunc( m_sequenceCache );
}
...
void CCSPlayerAnimState::CheckCachedSequenceValidity( void )
{
	if ( m_cachedModelIndex != GetOuter()->GetModelIndex() )
	{
		...
		{
			m_sequenceCache.Insert(ACT_HOP, GetOuter()->SelectWeightedSequence( ACT_HOP ));
			m_sequenceCache.Insert(ACT_IDLE, GetOuter()->SelectWeightedSequence( ACT_IDLE ));
			m_sequenceCache.Insert(ACT_RUN_CROUCH, GetOuter()->SelectWeightedSequence( ACT_RUN_CROUCH ));
			m_sequenceCache.Insert(ACT_CROUCHIDLE, GetOuter()->SelectWeightedSequence( ACT_CROUCHIDLE ));
			m_sequenceCache.Insert(ACT_RUN, GetOuter()->SelectWeightedSequence( ACT_RUN ));
			m_sequenceCache.Insert(ACT_WALK, GetOuter()->SelectWeightedSequence( ACT_WALK ));
			m_sequenceCache.Insert(ACT_IDLE, GetOuter()->SelectWeightedSequence( ACT_IDLE ));
		}
	}
}
...
int CCSPlayerAnimState::SelectWeightedSequence( Activity activity )
{
	...
	int idx = m_sequenceCache.Find(activity);
	if ( idx != m_sequenceCache.InvalidIndex() )
	{
		sequence = m_sequenceCache[idx];
	}
	else
	{
		sequence = GetOuter()->SelectWeightedSequence( activity );
		m_sequenceCache.Insert(activity, sequence);
	}
