...
{
	V_strncpy(m_szDefaultCreateDirName, defaultCreateDirName, sizeof(m_szDefaultCreateDirName));
...
void DirectorySelectDialog::ExpandTreeNode(const char *path, int parentNodeIndex)
{
	...
	V_snprintf(searchString, sizeof(searchString), "%s*.*", path);
...
{
	char searchString[512];
	V_snprintf(searchString, sizeof(searchString), "%s%s\\*.*", path, dir);
