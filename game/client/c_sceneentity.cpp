...
CChoreoScene *C_SceneEntity::LoadScene( const char *filename, IChoreoEventCallback *callback )
{
	...
	if ( bufsize > 0 )
	{
		...
		{
			pScene = new CChoreoScene( callback );
	...
	else if (filesystem->ReadFileEx( loadfile, "MOD", &pBuffer, true ))
	{
		...
		pScene = ChoreoLoadScene( loadfile, callback, &g_TokenProcessor, Scene_Printf );
	}
	else
	{
		// Abandon ship
	...
	if(pScene)
	{
		...
		pScene->SetEventCallbackInterface( callback );
	}
