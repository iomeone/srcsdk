...
// 		return;
	...
	V_snprintf( txt.m_chActivity, sizeof(txt.m_chActivity), "%s", seqdesc.pszActivityName() );
	V_snprintf( txt.m_chLabel, sizeof(txt.m_chLabel), "%s", seqdesc.pszLabel() );
	...
	if ( !txt.m_chActivity[0] )
	{
		...
		for ( int iLast = mpi.m_arrTxt.Count(); iLast --> 0; )
		{
			...
			{
				V_snprintf( txt.m_chActivity, sizeof(txt.m_chActivity), "%s", lastSeenTxt.m_chActivity );
				break;
			}
		}
	}
	...
	V_snprintf( txt.m_chTextLines[numLines], sizeof(txt.m_chTextLines[numLines]),
		"%-*s  %-*s  %*.2f  %*.1f/%-*d  %*.0f%% ",
		widthActivity,
	...
	{
		V_snprintf( chBuffer, sizeof(chBuffer),
			"iklocks : %-2d : ",
			seqdesc.numiklocks );
		...
		for ( int k = 0; k < seqdesc.numiklocks; ++ k )
		{
			...
			V_snprintf( chBuffer + strlen( chBuffer ), sizeof(chBuffer), "%s ", pchain->pszName() );
			// plock->flPosWeight;
	...
	{
		V_snprintf( chBuffer, sizeof(chBuffer), "ikrules : %-2d",
			seqdesc.numikrules );
