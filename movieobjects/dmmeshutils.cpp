	...
	const CUtlVector< int > &newToOldIndexMap )
{
	...
	for ( int i = 0; i < nFieldCount; ++i )
	{
		...
		CDmrGenericArray array( pVertexData->GetVertexData( i ) );
		RemoveUnusedData( pMesh, pVertexData, bBind, pFieldName, pNewVertexIndices, nNewToOldIndexMapCount, array );
	...
	for ( int i = 0; i < nOrigVertexCount; ++i )
	{
		if ( mirrorMap[ i ] != i )
		{
			...
			if ( dataMirrorMap[ origIndices[ i ] ] != origIndices[ i ] )
			{
				...
				const T_t &data( origData[ origIndices[ i ] ] );
				mirrorData = data;
	...
	int &nWrinkleOffset )
{
	...
	for ( int i = 0; i < pSrcBase->FieldCount(); ++i )
	{
		...
		for ( int j = 0; j < pDstBase->FieldCount(); ++j )
		{
			...
			case AT_FLOAT_ARRAY:
			{
				CDmrArray< float > array( pDstData );
				AppendData( CDmrArrayConst< float >( pSrcData ), array );
				break;
			}
			case AT_VECTOR2_ARRAY:
			{
				CDmrArray< Vector2D > array( pDstData );
				AppendData( CDmrArrayConst< Vector2D >( pSrcData ), array );
				break;
			}
			case AT_VECTOR3_ARRAY:
			{
				CDmrArray< Vector > array( pDstData );

				if ( i == nSrcPositionIndex )
				{
					AppendData( CDmrArrayConst< Vector >( pSrcData ), array, &pMat );
				}
				else if ( i == nSrcNormalIndex )
				{
					AppendData( CDmrArrayConst< Vector >( pSrcData ), array, &nMat );
				}
				else
				{
					AppendData( CDmrArrayConst< Vector >( pSrcData ), array );
				}
				break;
			}
			case AT_VECTOR4_ARRAY:
			{
				CDmrArray< Vector4D > array( pDstData );
				AppendData( CDmrArrayConst< Vector4D >( pSrcData ), array );
				break;
			}
			case AT_QUATERNION_ARRAY:
			{
				CDmrArray< Quaternion > array( pDstData );
				AppendData( CDmrArrayConst< Quaternion >( pSrcData ), array );
				break;
			}
			case AT_COLOR_ARRAY:
			{
				CDmrArray< Color > array( pDstData );
				AppendData( CDmrArrayConst< Color >( pSrcData ), array );
				break;
			}
			default:
				Assert( 0 );
