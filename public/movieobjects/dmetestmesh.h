...
struct vertex_t
{
	...
	Vector normal;
	Vector2D texcoord;
	CUtlVector< skinning_info_t > skinning;
...
struct submesh_t
{
	submesh_t( const CUtlString &texture_name ) : texname( texture_name )
	{
	}
	CUtlString texname;
	CUtlVector< int > indices;
	CUtlVector< vertex_t > vertices;
	CUtlVector< CDmeTransform* > bones;
};
...
public:
	...
	static CDmeTestMesh *ReadMeshFromSMD( const char *pFilename, DmFileId_t fileid );
...
private:
	...
	CUtlVector< submesh_t* > m_submeshes;
	CUtlVector< CDmeTransform* > m_bones;
};
