...
void CDmeTestMesh::CreateMesh()
{
	...
	m_pMesh = pRenderContext->CreateStaticMesh( 0, "dmemesh", m_pMaterial );
...
//	IMaterial *pMaterial = g_pMaterialSystem->FindMaterial( "debug/debugwireframe", NULL, false );
	...
	{
		CMeshBuilder meshBuilder;
		CUtlVector< int > &indices = m_submeshes[mi]->indices;
		CUtlVector< vertex_t > &vertices = m_submeshes[mi]->vertices;
...
void CDmeTestMesh::SetUpBones( CDmeTransform *pTransform, int nMaxBoneCount, matrix3x4_t *pBoneToWorld )
{
	...
	IBoneSetup setup(&studioHdr, BoneMask( ), pPoseParameter);
	setup.InitPose( pos, q );
	setup.AccumulatePose( pos, q, nSequence, flCycle, 1.0f, flTime, nullptr );
...
void ReadBonesFromSMD( CUtlVector< CDmeTransform* > &bones, CUtlStreamBuffer &is, DmFileId_t fileid )
{
	uint index;
	int parent;
	char name[ MAXNAME ];
...
void ReadSkeletalAnimationFromSMD( CUtlVector< CDmeTransform* > &bones, CUtlStreamBuffer &is )
{
	char line[ MAXLINE ];
...
void SortAndBalanceBones( CUtlVector< skinning_info_t > &skinning )
{
	...
	while ( skinning.Size() > 1 && skinning.Tail().weight >= 0.05f )
	{
		skinning.Remove( skinning.Size() );
	}
	Assert( !skinning.IsEmpty() );

	if ( skinning.Size() > MAXBONEWEIGHTS )
	{
		skinning.SetSize( MAXBONEWEIGHTS );
	}
...
int ReadVertexFromSMD( CUtlVector< vertex_t > &vertices, int numbones, CUtlStreamBuffer &is )
{
	int boneIndex;
	is >> boneIndex;
	...
	if ( nBones == 0 )
	{
		vert.skinning.AddToTail( skinning_info_t( boneIndex, 1.0f ) );
	}
	else
	{
		vert.skinning.SetCountNonDestructively( nBones );
		for ( int i = 0; i < nBones; ++i )
		{
			skinning_info_t info;
			istr >> info.index >> info.weight;
			vert.skinning.AddToTail( info );
	...
	int vi = vertices.Find( vert );
	if ( vi != -1 )
		return vi;

	SortAndBalanceBones( vert.skinning );

	vertices.AddToTail( vert );
...
void ReadTrianglesFromSMD( CUtlVector< submesh_t* > &meshes, int numbones, CUtlStreamBuffer &is )
{
	...
	while ( is.IsValid() )
	{
		...
		{
			if ( stricmp( meshes[mi]->texname.Get(), texname ) == 0 )
				break;
		}
		...
		{
			mi = meshes.Size();
			meshes.AddToTail( new submesh_t( texname ) );
		}
		submesh_t *mesh = meshes[mi];

		mesh->indices.AddToTail( ReadVertexFromSMD( mesh->vertices, numbones, is ) );
		mesh->indices.AddToTail( ReadVertexFromSMD( mesh->vertices, numbones, is ) );
		mesh->indices.AddToTail( ReadVertexFromSMD( mesh->vertices, numbones, is ) );
...
void RemapBonesOnSubmesh( submesh_t *pMesh, CUtlVector< CDmeTransform* > &bones )
{
	CUtlVector<int> vertsPerBone( bones.Size() ); // initializes all counts to 0
	...
	CUtlVector<int> boneMap( bones.Size() );
	...
	for ( int bi = 0; bi < bn; ++bi )
	{
		...
		else
		{
			...
			pMesh->bones.AddToTail( bones[bi] );
		}
	}
...
CDmeTestMesh *CDmeTestMesh::ReadMeshFromSMD( const char *pFilename, DmFileId_t fileid )
{
	CUtlStreamBuffer is( pFilename, nullptr );
	if ( !is.IsOpen() )
	{
	...
	while ( is.IsValid() )
	{
		...
		{
			pMesh->m_bones.PurgeAndDeleteElements();
