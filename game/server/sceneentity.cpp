...
public:
	...
	bool ShouldFindEntity( CBaseEntity *pEntity )
	{
		...
		UTIL_TraceHull( vecOrigin, vecOrigin, m_hActor->WorldAlignMins(), m_hActor->WorldAlignMaxs(), MASK_SOLID, m_hActor, COLLISION_GROUP_NONE, &tr );
		if ( tr.startsolid )
		{
			return false;
		}
...
CBaseEntity *CSceneEntity::FindNamedEntity( const char *name, CBaseEntity *pActor, bool bBaseFlexOnly, bool bUseClear )
{
	...
	else if (pActor && pActor->IsNPC())
	{
		...
		entity = pActor->MyCombatCharacterPointer()->FindNamedEntity( name, pFilter );
		if ( !entity && pFilter )
		{
...
CBaseEntity *CInstancedSceneEntity::FindNamedEntity( const char *name )
{
	...
	if (m_hOwner != NULL)
	{
		CBaseCombatCharacter	*npc = m_hOwner->MyCombatCharacterPointer();
