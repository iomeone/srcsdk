...
ConVar cl_autohelp(
	...
	float CCSGameRules::GetAmountOfEntityVisible(Vector & vecSrc, CBaseEntity *entity)
	{
		...
		float armDistanceFromChest = 24.0f;
	 ...
	 */
	int UTIL_HumansOnTeam( int teamID, bool isAlive = true )
	{
		int count = 0;
...
void DrawSphere( const Vector& pos, float radius, int r, int g, int b, float lifetime )
{
	...
	{
		edge.x = radius * cos( angle ) + pos.x;
		edge.y = pos.y;
		edge.z = radius * sin( angle ) + pos.z;
