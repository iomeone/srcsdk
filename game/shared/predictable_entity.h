...
#define BEGIN_PREDICTION_DATA( className ) \
	datamap_t className::m_PredMap = datamap_t{ 0, 0, #className, &BaseClass::m_PredMap }; \
...
#define BEGIN_PREDICTION_DATA_NO_BASE( className ) \
	datamap_t className::m_PredMap = datamap_t{ 0, 0, #className, NULL }; \
	datamap_t *className::GetPredDescMap( void ) { return &m_PredMap; } \
	BEGIN_PREDICTION_DATA_GUTS( className )
...
#define BEGIN_PREDICTION_DATA( className ) \
	...
	{ \
		...
		{ \
			...
			{ \
				typedescription_t{ FIELD_VOID,0, typedesc_offset_t{0,0},0,0,0,0,0,0},
