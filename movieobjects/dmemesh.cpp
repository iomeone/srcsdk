		...
		for ( int j( 0 ); j < nBaseField; ++j )
		{
			...
			for ( int k( 0 ); k < nDeltaField; ++k )
			{
				...
				case AT_FLOAT_ARRAY:
				{
					...
					AddCorrectedDelta( array, baseIndices, compList[ i ], baseFieldName );
					break;
				}
				case AT_COLOR_ARRAY:
				{
	...
	const CDmeSingleIndexedComponent *pMask ) const
{
	...
	for ( int i = 0; i < naField; ++i )
	{
		...
		for ( int j = 0; j < nbField; ++j )
		{
			...
			case AT_FLOAT_ARRAY:
			{
				CDmrArray< float > array( paAttr );
				InterpMaskedData( array, CDmrArrayConst< float >( pbAttr ).Get(), weight, pMask );
				break;
			}
			case AT_COLOR_ARRAY:
			{
				CDmrArray< Vector4D > array( paAttr );
				InterpMaskedData( array, CDmrArrayConst< Vector4D >( pbAttr ).Get(), weight, pMask );
				break;
			}
			case AT_VECTOR2_ARRAY:
			{
				CDmrArray< Vector2D > array( paAttr );
				InterpMaskedData( array, CDmrArrayConst< Vector2D >( pbAttr ).Get(), weight, pMask );
				break;
			}
			case AT_VECTOR3_ARRAY:
			{
				CDmrArray< Vector > array( paAttr );
				InterpMaskedData( array, CDmrArrayConst< Vector >( pbAttr ).Get(), weight, pMask );
				break;
			}
			default:
				break;
			...
			for ( int k = 0; k < nBindField; ++k )
			{
				...
				for ( int l = 0; l < nDeltaField; ++l )
				{
					...
					case AT_FLOAT_ARRAY:
					{
						...
						CDmrArray< float > array( pDstBaseData );
						InterpMaskedData( array, floatData, weight, pMask );
						break;
					}
					case AT_COLOR_ARRAY:
					{
						...
						CDmrArray< Vector4D > array( pDstBaseData );
						InterpMaskedData( array, vector4DData, weight, pMask );
						break;
					}
					case AT_VECTOR2_ARRAY:
					{
						...
						CDmrArray< Vector2D > array( pDstBaseData );
						InterpMaskedData( array, vector2DData, weight, pMask );
						break;
					}
					case AT_VECTOR3_ARRAY:
					{
						...
						CDmrArray< Vector > array( pDstBaseData );
						InterpMaskedData( array, vectorData, weight, pMask );
						break;
					}
					default:
						break;
...
bool CDmeMesh::SetBaseDataToDeltas(
	CDmeVertexData *pBase,
	CDmeVertexData::StandardFields_t nStandardField, const CDmrArrayConst< T_t > &srcData, CDmrArray< T_t > &dstData, bool bDoStereo, bool bDoLag )
{
...
bool CDmeMesh::SetBaseStateToDeltas( CDmeVertexData *pPassedBase /*= NULL */ )
{
	...
	for ( int i = 0; i < sizeof( deltaFields ) / sizeof( deltaFields[ 0 ] ); ++i )
	{
		...
		case AT_FLOAT_ARRAY:
		{
			CDmrArray< float > array( pDstAttr );
			SetBaseDataToDeltas( pBind, nStandardField, CDmrArrayConst< float >( pSrcAttr ), array, bDoStereo, false );
			break;
		}
		case AT_VECTOR3_ARRAY:
		{
			CDmrArray< Vector > array( pDstAttr );
			SetBaseDataToDeltas( pBind, nStandardField, CDmrArrayConst< Vector >( pSrcAttr ), array, bDoStereo, false );
			break;
		}
		default:
			Assert( 0 );
