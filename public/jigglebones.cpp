...
ConVar cl_jiggle_bone_sanity( "cl_jiggle_bone_sanity", "1", 0, "Prevent jiggle bones from pointing directly away from their target in case of numerical instability." );
ConVar cl_jiggle_bone_invert( "cl_jiggle_bone_invert", "0", FCVAR_CHEAT );
...
void CJiggleBones::BuildJiggleTransformations( int boneIndex, float currenttime, const mstudiojigglebone_t *jiggleInfo, const matrix3x4_t &goalMX, matrix3x4_t &boneMX )
{
	...
	if ( cl_jiggle_bone_invert.GetBool() )
	{
...
#ifdef CLIENT_DLL
	if ( data->id == cl_jiggle_bone_debug.GetInt() )
	{
		int line = 20;
	...
	if ( cl_jiggle_bone_debug.GetInt() == 1 || cl_jiggle_bone_debug.GetInt() == data->id )
	{
		float dT = 0.01f;
