...
void CBaseAnimationSetEditor::ImportAnimation( CDmeChannelsClip *pChannelsClip, bool bVisibleOnly )
{
	...
	CDmeChannel** ppFoundChannels = (CDmeChannel**)stackalloc( nSrcCount * sizeof(CDmeChannel*) );
	int nFoundCount = 0;
	...
	for ( int i = 0; i < nCount; ++i )
	{
		for ( int k = 0; k < LOG_PREVIEW_MAX_CHANNEL_COUNT; ++k )
		{
			...
			Q_snprintf( pImportInfo, sizeof(pImportInfo), "\"%s\" : %s", pImportChannel->GetToElement()->GetName(), pImportChannel->GetToAttribute()->GetName() );
		}
	}
	...
	for ( int i = 0; i < nSrcCount; ++i )
	{
		...
		char pImportInfo[512];
		Q_snprintf( pImportInfo, sizeof(pImportInfo), "\"%s\" : %s", pMissingChannel->GetToElement()->GetName(), pMissingChannel->GetToAttribute()->GetName() );
	}
