...
void IGameSystem::Add( IGameSystem* pSys )
{
	if( !pSys )
	{
		DebugPrintf("Add: Passed a NULL IGameSystem!\n");
	}

	DebugPrintf("Adding to list(%i): %s\n", s_GameSystems.Count(), pSys->Name() );
	s_GameSystems.AddToTail( pSys );

#if 1
	if( pSys->IsPerFrame() )
	{
		DebugPrintf("Add Per Frame: %s\n", pSys->Name() );
...
bool IGameSystem::InitAllSystems()
{
	...
	for ( i = 0; i < s_GameSystems.Count(); ++i )
	{
		...
		char sz[128];
		Q_snprintf( sz, sizeof( sz ), "%s->Init():Start\n", sys->Name() );
		DebugPrintf( "%s", sz );

		bool valid = sys->Init();

		Q_snprintf( sz, sizeof( sz ), "%s->Init():Finish\n", sys->Name() );
		DebugPrintf( "%s", sz );
