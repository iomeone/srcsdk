...
void CGamePlayerZone::InputCountPlayersInZone( inputdata_t &inputdata )
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer )
		{
			...
			{
				mins = VEC_DUCK_HULL_MIN_SCALED(pPlayer);
				maxs = VEC_DUCK_HULL_MIN_SCALED(pPlayer);
			}
			else
			{
				mins = VEC_HULL_MIN_SCALED(pPlayer);
				maxs = VEC_HULL_MIN_SCALED(pPlayer);
			}

			UTIL_TraceModel( pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin(), mins, maxs, this, COLLISION_GROUP_NONE, &trace );
