...
void C_OP_BasicMovement::Operate( CParticleCollection *pParticles, float flStrength, void *pContext ) const
{
	...
	fltx4 adj_dt = ReplicateX4( ( pParticles->m_flDt / pParticles->m_flPreviousDt ) * ExponentialDecay( (1.0f-max(0.0f,m_fDrag)), (1.0f/30.0f), pParticles->m_flDt ) );
							 ...
							 pParticles->m_pDef->m_Constraints[i] ) ) )
					{
						...
						bool bDidSomething=
							pParticles->m_pDef->m_Constraints[i]->EnforceConstraint(
								...
								pParticles->m_pOperatorContextData + 
								pParticles->m_pDef->m_nConstraintsCtxOffsets[i], 0 );
						END_OP;
						CHECKSYSTEM( pParticles );
						if ( bDidSomething )
						{
